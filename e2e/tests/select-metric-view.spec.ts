...
test.describe('Select metric view', () => {
  ...
  test('Copy url', async ({ selectMetricView, page }) => {
    ...
    const expectedContent = `${getGrafanaUrl()}/a/grafana-metricsdrilldown-app/trail?metric=a.utf8.metric%20%F0%9F%A4%98&nativeHistogramMetric=&useOtelExperience=1&trailActivated=1&&from=now-15m&to=now&timezone=browser&var-ds=gdev-prometheus&var-otel_resources=&var-filters=&var-otel_and_metric_filters=&var-deployment_environment=undefined&var-variant=onboard-filters-sidebar&var-labelsWingman=&actionView=breakdown&var-groupby=$__all&breakdownLayout=grid`;
